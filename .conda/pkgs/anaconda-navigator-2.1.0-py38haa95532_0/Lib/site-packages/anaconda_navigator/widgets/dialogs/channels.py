...
class ListWidgetChannels(QListWidget):
    ...
    def __init__(
        ...
        if logged_api_url and logged_api_url != CONF.get('main', 'anaconda_api_url'):
    ...
    def validate_channel(self, item):
        ...
        if channel.startswith('file:///'):
            ...
            worker = WorkerMock(item=item, url=channel, repodata_url=channel)
            worker.channel = channel
            path = channel.replace('file:///', '') if os.name == 'nt' else channel.replace('file://', '')
        ...
        if channel.startswith(('https://', 'http://')):
            ...
            if logged_api_url and logged_api_url == team_edition_api_url:
                url = parse.urljoin(self.main_url, "api/repo/t/{0}/{1}".format(token, channel))
...
class DialogChannels(DialogBase):
    ...
    def setup(self, worker, conda_config_data, error):
        ...
        self.config_sources = conda_config_data.get('config_sources', list())
