...
class _CondaAPI(QObject):
    ...
    def export_environment(
        ...
        # Base command
        cmd_list: typing.Sequence[str] = ['env', 'export']
        ...
        logger.debug('Conda export_environment cmd: %s', cmd_list)

        # Execution
        return self._call_conda(cmd_list)  # without parsing, as `--json` breaks this command

    def _create_from_yaml(self, yamlfile, prefix=None, name=None, offline=False):
        """
    ...
    def create(
        ...
        if file and file.endswith(('.yaml', '.yml')):
            result = self._create_from_yaml(file, prefix=prefix, name=name, offline=offline)
        else:
    ...
    def get_repodata(self, channels=None, pkgs_dirs=None, append_mtime: bool = False):
        ...
        for pkgs_dir in official_pkgs_dirs:
            ...
            if os.path.isdir(cache):
                paths = [os.path.join(cache, path) for path in os.listdir(cache)]
                all_paths.extend(
                    path
                    for path in paths
        ...
        if append_mtime:
            ...
            if mod_dates:
                result_mtime = max(mod_dates.values())
            return repodata_dic, result_mtime
        return repodata_dic
