...
from anaconda_navigator.config import (
    ...
    REMOVED_LICENSE_PATH, VALID_PRODUCT_LICENSES, WIN,
    ANACONDA_ORG_BRAND, COMMERCIAL_EDITION_BRAND, TEAM_EDITION_BRAND, ENTERPRISE_EDITION_BRAND)
...
class _AnacondaAPI(QObject):
    ...
    def process_packages(self, packages, prefix=None, blacklist=()):
        ...
        def _pip_data_ready(worker, output, error):
            ...
            logger.debug('output: {}, error: {}'.format(pip_list_data, error))
            ...
            current_platform = self._conda_api.get_platform()
            metadata_channels = set("{}/{}".format(base_url,  current_platform) for base_url in channel_urls)
            metadata_channels |= set("{}/noarch".format(base_url) for base_url in channel_urls)
    ...
    def detect_commercial_edition_login(self):
        ...
        if any(True for token_domain in token_list().keys() if commercial_edition_base_url in token_domain):
            ...
            self.config.set('main', 'logged_brand', COMMERCIAL_EDITION_BRAND)
        ...
        # Try to check if token with team edition url (specified in anaconda-navigator.ini) is present.
            ...
            for api_url in tokens:
                if team_edition_api_url in api_url and tokens[api_url]:
                    ...
                    self.config.set('main', 'logged_brand', TEAM_EDITION_BRAND)
    ...
    def detect_anaconda_org_login(self):
        domain = self.client_domain()
        if domain and domain == self.config.get('main', 'anaconda_api_url'):
            self.config.set('main', 'logged_api_url', domain)
            self.config.set('main', 'logged_brand', ANACONDA_ORG_BRAND)

    def detect_enterprise_org_login(self):
        domain = self.client_domain()
        logged_api_url = self.config.get('main', 'logged_api_url')
        if domain and domain != self.config.get('main', 'anaconda_api_url') and not logged_api_url:
            self.config.set('main', 'logged_api_url', domain)
            self.config.set('main', 'logged_brand', ENTERPRISE_EDITION_BRAND)

    def __backup_and_replace_condarc(self, default_channels=None, channels=None, channel_alias=None):
        """
        Saves the copy of the .condarc data which was set before login action
         and replace it with duplicate with modified channels data.
        """
    ...
    def create_login_data(self, default_channels=None, channels=None):
        ...
        if isinstance(self._client_api.anaconda_client_api, TeamEditionAPI):
            channel_alias = '{}/api/repo'.format(self.config.get('main', 'team_edition_api_url'))
            self.__backup_and_replace_condarc(
                default_channels=default_channels, channels=channels, channel_alias=channel_alias)
        ...
        enterprise_edition_api_url = self.config.get(self.config.DEFAULT_SECTION_NAME, 'enterprise_4_repo_api_url')
        if logged_api_url and logged_api_url == enterprise_edition_api_url:
            channel_alias = "{}/conda".format(get_domain_from_api_url(enterprise_edition_api_url))
            self.__backup_and_replace_condarc(
                default_channels=default_channels, channels=channels, channel_alias=channel_alias)
